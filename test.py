...
class MyWXBot(WXBot):
	...
	def sendQRCodeEmail(self,content):
		scheduleTask.send_mail_with_image([],'请重新登录微信机器人账号',content)
	...
	def handleGroupTextMessage(self,msg):
		...
		if linkMatch:
			contentPath = os.path.join(self.checkGroupFolder(msg),'share_link'+datetime.date.today().strftime('%Y_%m_%d') + '.txt')
			f = open(contentPath,'a')
			f.write(content+'\n')
			f.close()
		elif keywordMatch:
			contentPath = os.path.join(self.checkGroupFolder(msg),'hot_keyword'+datetime.date.today().strftime('%Y_%m_%d') + '.txt')
			f = open(contentPath,'a')
	...
	def handleGroupCardMessage(self,msg):
		...
		contentPath = os.path.join(self.checkGroupFolder(msg),'public_card'+datetime.date.today().strftime('%Y_%m_%d') + '.txt')
		f = open(contentPath,'a')
