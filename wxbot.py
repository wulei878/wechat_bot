...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        """
        mtype = msg['MsgType']
        content = html.parser.unescape(msg['Content'])
        msg_id = msg['MsgId']
        ...
        if msg_type_id == 0:
            ...
            uid = content[:sp]
            content = content[sp:]
            result = re.search(r'(http|https)://', content, re.IGNORECASE)
            if result:
    ...
    def handle_msg(self, r):
        ...
        for msg in r['AddMsgList']:
            ...
            if not user['name']:
                user['name'] = 'unknown'
            user['name'] = html.parser.unescape(user['name'])
    ...
    def gen_qr_code(self, qr_file_path):
        ...
        if self.conf['qr'] == 'png':
            ...
            if self.isRemoteControl:
                self.sendQRCodeEmail(qr_file_path)
            else:
